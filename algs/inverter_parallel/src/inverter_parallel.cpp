...
CPLErr invertColorsSerial(float *buffer, AlgoData data) {
    ...
    error = data.inputBand->RasterIO(
        GF_Read, data.tileXo, data.tileYo, data.tileXa, data.tileYa, buffer,
        data.tileXa, data.tileYa, GDT_Float32, 0, 0);
    if (error) {
        return error;
    }
    for (i = 0; i < size; i++) {
    ...
    error = data.outputBand->RasterIO(
        GF_Write, data.tileXo, data.tileYo, data.tileXa, data.tileYa, buffer,
        data.tileXa, data.tileYa, GDT_Float32, 0, 0);
    return error;
}
...
int inverterParallelTimeTest(std::string const &fileName) {
    ...
    float *buffer =
        (float *)CPLMalloc(sizeof(float) * algoData.tileX * algoData.tileY);
    ...
    while (!erroredConfs->empty()) {
        ...
        if (error) {
            std::cerr << "Got gdal error for the second time: " << error
