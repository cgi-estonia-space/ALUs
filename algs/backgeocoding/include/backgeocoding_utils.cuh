...
inline __device__ bool GetPositionLUT(s1tbx::DeviceSubswathInfo* subswath_info,
                                   ...
                                   const int num_orbit_vec, const double dt, int idx, int idy, const double* osv_lut) {
    const double zero_doppler_time_in_days =
        s1tbx::sargeocoding::GetZeroDopplerTimeLUT(sentinel1_utils->line_time_interval, sentinel1_utils->wavelength,
                                                position_data->earth_point, orbit, num_orbit_vec, dt, osv_lut);
