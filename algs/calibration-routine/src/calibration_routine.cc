...
int CalibrationRoutine::Execute() {
    try {
        ...
        if (!wkt_aoi_.empty()) {
            split_op = std::make_unique<topsarsplit::TopsarSplit>(input_path, subswath, polarization, wkt_aoi_);
...
void CalibrationRoutine::SetEgm96Manager(snapengine::EarthGravitationalModel96* manager) { egm96_manager_ = manager; }
