...
[[nodiscard]] int CoherenceExecuter::Execute() {
    try {
        ...
        LOGE << "Caught exception while running coherence operation - " << e.what();
        return 3;
    } catch (...) {
        LOGE << "Caught unknown exception while running coherence operation.";
        return 2;
    }
...
void CoherenceExecuter::ExecuteSafe() {
    ...
    alus::MetaData meta_master{near_range_on_left, snapengine::AbstractMetadata::GetAbstractedMetadata(main_product_),
                               orbit_degree_};
    alus::MetaData meta_slave{near_range_on_left,
                              snapengine::AbstractMetadata::GetAbstractedMetadata(secondary_product_), orbit_degree_};
    ...
    BandParams band_params{band_map_out,
                           ...
                           coh_data_reader->GetBandXMin(),
                           coh_data_reader->GetBandYMin()};
    coh_data_writer = std::make_unique<GdalTileWriter>(output_name_, band_params, coh_data_reader->GetGeoTransform(),
...
void CoherenceExecuter::SetParameters(const app::AlgorithmParameters::Table& param_values) {
    ...
    if (const auto mem_fraction = param_values.find(std::string(PARAMETER_PER_PROCESS_GPU_MEMORY_FRACTION));
        ...
        if (std::isless(value, 0.0) || std::isgreater(value, 1.0) || std::isnan(value)) {
            LOGE << PARAMETER_PER_PROCESS_GPU_MEMORY_FRACTION << " value (" << value << ") out of range [0.0...1.0]";
        } else {
    ...
    if (band_map_in != param_values.end()) {
        ...
        if (value.length() != 4) {
            LOGE << PARAMETER_INPUT_BAND_MAP << " value (" << value
                 << ") not in correct format - expecting 4 integers.";
        } else {
...
[[nodiscard]] std::string CoherenceExecuter::GetArgumentsHelp() const {
    ...
    help_stream << "Coherence configuration options:"
                ...
                << " - azimuth window size in pixels (default:" << coherence_window_azimuth_ << ")"
                ...
                << per_process_fpu_memory_fraction_ << ")";
    // clang-format on
...
void CoherenceExecuter::PrintProcessingParameters() const {
    std::stringstream bands;
    for (const auto& band : band_map_in_) {
        bands << band << " ";
    }

    // clang-format off
    LOGI << "Coherence processing parameters:"
              << PARAMETER_ID_SRP_NUMBER_POINTS << " " << srp_number_points_
              << PARAMETER_ID_SRP_POLYNOMIAL_DEGREE << " " << srp_polynomial_degree_
              << PARAMETER_ID_SUBTRACT_FLAT_EARTH_PHASE << " " << (subtract_flat_earth_phase_ ? "true" : "false")
              << PARAMETER_ID_RG_WINDOW << " " << coherence_window_range_
              << PARAMETER_ID_AZ_WINDOW << " " << coherence_window_azimuth_
              << PARAMETER_ID_ORBIT_DEGREE << " " << orbit_degree_
              << PARAMETER_FETCH_TRANSFORM << " " << (fetch_transform_ ? "true" : "false")
              << PARAMETER_PER_PROCESS_GPU_MEMORY_FRACTION << " " << per_process_fpu_memory_fraction_
              << PARAMETER_INPUT_BAND_MAP << " " << bands.str();
    // clang-format on
}
}  // namespace alus
