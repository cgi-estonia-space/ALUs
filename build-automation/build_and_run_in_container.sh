...
if [ "$#" -lt 2 ]; then
    echo "$0 "
    echo "Usage:"
    echo "$0 [ALUs CI package] [artifacts and test results output location] <resources folder - optional>"
    exit 1
fi
...
docker_image_name="cgialus/alus-devel:jammy"
...
docker run -t -d --gpus all --name $container_name $docker_image_name
...
set +e
docker exec -t $container_name bash -c "cd $container_work_dir; CUDAARCHS=75 build-automation/build_and_run_ci.sh"
tests_return_value1=$?
set -e
...
set +e
docker exec -t $container_name bash -c "cd $container_work_dir; CC=clang CXX=clang++ CUDAARCHS=75 build-automation/build_and_run_ci.sh"
tests_return_value2=$?
set -e
