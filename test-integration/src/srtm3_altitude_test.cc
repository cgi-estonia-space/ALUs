...
using alus::tests::SRTM3TestData;
...
public:
    ...
    explicit Srtm3AltitudeTester(std::string_view test_file_name) { this->test_file_name_ = test_file_name; }
...
TEST(SRTM3, altitudeCalc) {
    ...
    dim3 grid_size(alus::cuda::GetGridDim(static_cast<int>(block_size.x), static_cast<int>(tester.size_)));
    ...
    size_t count = alus::EqualsArraysd(tester.end_results_.data(), tester.alts_.data(), static_cast<int>(tester.size_),
                                       error_margin);
