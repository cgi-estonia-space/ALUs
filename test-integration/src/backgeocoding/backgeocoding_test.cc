...
public:
    ...
    BackgeocodingTester(std::string_view q_phase_file, std::string_view i_phase_file, std::string_view q_tile_file,
                        std::string_view i_tile_file) {
    ...
    void ReadTile(alus::Rectangle area, int16_t* tile_i, int16_t* tile_q) const {
...
TEST(backgeocoding, correctness) {
    ...
    size_t tile_size;
    const alus::Rectangle target_area_land = {4000, 17000, 100, 100};
    const int m_burst_index_land = 11;
    const int s_burst_index_land = m_burst_index_land + burst_offset;
    tile_size = static_cast<size_t>(target_area_land.width) * static_cast<size_t>(target_area_land.height);
    ...
    const float error_margin{0.00001};
    size_t count_i =
        alus::EqualsArrays(i_results_land.data(), land_tester.i_result_.data(), land_tester.tile_size_, error_margin);
    EXPECT_EQ(count_i, count_i != 0) << "Land results I do not match. Mismatches: " << count_i << '\n';

    size_t count_q =
        alus::EqualsArrays(q_results_land.data(), land_tester.q_result_.data(), land_tester.tile_size_, error_margin);
    EXPECT_EQ(count_q, count_q != 0) << "Land results Q do not match. Mismatches: " << count_q << '\n';

    const alus::Rectangle target_area_sea{10, 19486, 50, 50};
    const int m_burst_index_sea = 12;
    const int s_burst_index_sea = m_burst_index_sea + burst_offset;
    ...
    const alus::Rectangle target_area_coast{4700, 21430, 100, 100};
    const int m_burst_index_coast = 14;
    const int s_burst_index_coast = m_burst_index_coast + burst_offset;
    tile_size = static_cast<size_t>(target_area_coast.width) * static_cast<size_t>(target_area_coast.height);
    ...
    // Sometimes we get 1 mismatch, by about 0.0001. It's checked and fine. Debug if there are 2 or more
    size_t count_i_coast = alus::EqualsArrays(i_results_coast.data(), coast_tester.i_result_.data(),
                                              coast_tester.tile_size_, error_margin);
    EXPECT_EQ(count_i_coast, count_i_coast != 0)
        << "Coast results I do not match. Mismatches: " << count_i_coast << '\n';

    size_t count_q_coast = alus::EqualsArrays(q_results_coast.data(), coast_tester.q_result_.data(),
                                              coast_tester.tile_size_, error_margin);
