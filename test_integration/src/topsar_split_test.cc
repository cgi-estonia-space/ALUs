...
TEST(topsar_split, s1utils_slave){
    ...
    size_t doppler_count =
        alus::EqualsArrays2Dd(slave_utils.subswath_.at(0)->doppler_rate_, tester.doppler_rate_2_,
    ...
    size_t reference_count =
        alus::EqualsArrays2Dd(slave_utils.subswath_.at(0)->reference_time_, tester.reference_time_2_,
                              slave_utils.subswath_.at(0)->num_of_bursts_, slave_utils.subswath_.at(0)->samples_per_burst_);
    EXPECT_EQ(reference_count, 0) << "Reference Times do not match. Mismatches: " << reference_count << '\n';

    size_t range_doppler_count =
        alus::EqualsArrays2Dd(slave_utils.subswath_.at(0)->range_depend_doppler_rate_, tester.range_depend_doppler_rate_2_,
                              slave_utils.subswath_.at(0)->num_of_bursts_, slave_utils.subswath_.at(0)->samples_per_burst_);
    EXPECT_EQ(range_doppler_count, 0) << "Range Dependent Doppler Rates do not match. Mismatches: "
                                      << range_doppler_count << '\n';

    size_t centroids_count =
        alus::EqualsArrays2Dd(slave_utils.subswath_.at(0)->doppler_centroid_, tester.doppler_centroid_2_,
                              slave_utils.subswath_.at(0)->num_of_bursts_, slave_utils.subswath_.at(0)->samples_per_burst_);
...
TEST(topsar_split, s1utils_master){
    ...
    std::string subswath_name = "IW1";
    std::string polarisation = "VV";
    std::string master_file = "/home/erik/snapDebusTests/BEIRUT/S1B_IW_SLC__1SDV_20200730T034254_20200730T034321_022695_02B131_E8DD.SAFE";
