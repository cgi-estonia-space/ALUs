...
protected:
    ...
    void run_single_burst_data_2018_test(const std::string& file_name_out, int tile_x_size, int tile_y_size,
                                         std::string& band_hash) {
        {
            ...
            const int TILE_X_SIZE{tile_x_size};
            const int TILE_Y_SIZE{tile_y_size};
            ...
            float avg_incidence_angle = ia_data_reader.GetValueAtXy(ia_width / 2, ia_height / 2);
...
TEST_F(CoherenceIntegrationTest, single_burst_data_2018) {
    ...
    for (int i = 0; i < 5; i++) {
        std::string band_hash;
        run_single_burst_data_2018_test(file_out, tile_x.at(i), tile_y.at(i), band_hash);
        if (i > 0) {
