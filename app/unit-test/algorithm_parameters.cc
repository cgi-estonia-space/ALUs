...
using alus::app::AlgorithmParameters;
...
TEST(AlgorithmParametersTest, parsesCorrectlySingleEntityParameters) {
    ...
    ASSERT_THAT(test.GetParameters(),
                UnorderedElementsAre(Pair("param", "value"), Pair("param2", "2"), Pair("param3", "value"),
                                     Pair("tere", "mistoimps"), Pair("weird", "val ue")));
}
...
TEST(AlgorithmParametersTest, parsesCorrectlyParametersWithoutAlgorithmName) {
    ...
    ASSERT_THAT(test.GetParameters(),
                UnorderedElementsAre(Pair("first", "1"), Pair("second", "2"), Pair("param3", "5456.0932"),
                                     Pair("par am", "1e-6"), Pair("last_one", "OK12")));
}
...
TEST(AlgorithmParametersTest, parsesCorrectlyParametersWithoutAlgorithmNameWithWkt) {
    ...
    ASSERT_THAT(
        test.GetParameters(),
        UnorderedElementsAre(Pair("first", "yes"), Pair("second", "2"), Pair("param3", "5.930"), Pair("par am", "1e-6"),
...
TEST(AlgorithmParametersTest, parsesCorrectlyMulipleAlgorithmParameters) {
    ...
    EXPECT_THAT(results.at("coherence"), UnorderedElementsAre(Pair("subtract_flat_earth_phase", "false"),
                                                              Pair("rg", "65.90"), Pair("az", "140")));
...
TEST(AlgorithmParametersTest, parsesCorrectlyMultipleAlgorithmParametersFromFile) {
    ...
    const auto temp_file = std::experimental::filesystem::temp_directory_path() / parameter_file_name;
...
TEST(AlgorithmParametersTest, mergesDuplicateParametersWithWarnings) {
    ...
    EXPECT_THAT(warnings, HasSubstr("terrain-correction parameter 'useAvgSceneHeight' redeclared on command line, "
...
TEST(AlgorithmParametersTest, mergesParametersWithoutWarnings) {
    ...
    ASSERT_THAT(results, SizeIs(5));
    EXPECT_THAT(
        results.at("terrain-correction"),
        UnorderedElementsAre(Pair("useAvgSceneHeight", "true"), Pair("points", "10"), Pair("avgSceneHeight", "10")));
    EXPECT_THAT(results.at("coherence"),
