...
void check(T result, char const* const func, const char* const file, int const line) {
    if (result) {
        std::string error_message = "CUDA error at " + std::string(file) + ":" + std::to_string(line) +
                                    " code=" + std::to_string(static_cast<unsigned int>(result)) + "(" +
                                    std::string(_cudaGetErrorEnum(result)) + ")" + " \"" + std::string(func) + "\"";
        throw alus::CudaErrorException(error_message);
    }
}
