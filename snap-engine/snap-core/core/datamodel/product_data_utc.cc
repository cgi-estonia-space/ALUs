...
Utc::Utc(double mjd) : UInt(3) {  // NOLINT
    ...
    SetElemUIntAt(0, static_cast<int64_t>(days));
    SetElemUIntAt(1, static_cast<int64_t>(seconds));
    SetElemUIntAt(2, static_cast<int64_t>(micro_seconds));
}
...
std::shared_ptr<Utc> Utc::Create(boost::posix_time::ptime date_time, int64_t micros) {
    ...
    int millis_per_second = 1000;                           // NOLINT
    int millis_per_day = 24 * 60 * 60 * millis_per_second;  // NOLINT
    int64_t days = diff / millis_per_day;
    int64_t seconds = (diff - days * millis_per_day) / millis_per_second;
...
//}
boost::posix_time::ptime Utc::GetAsDate() const { return GetAsCalendar(); }

boost::gregorian::date Utc::CreateCalendar() {
    // todo: might need to add some custom locale (check java impl)

    return boost::gregorian::date{2000, 1, 1};  // NOLINT
}
...
boost::posix_time::ptime Utc::GetAsCalendar() const {
    ...
    auto millis =
        boost::posix_time::milliseconds(static_cast<int>(std::round(GetMicroSecondsFraction() / 1000.0)));  // NOLINT
...
std::string Utc::Format() const { return Format(DATE_FORMAT_PATTERN); }
...
std::string Utc::Format(std::string_view format) const {
    ...
    for (int i = micros_string.length(); i < 6; i++) {  // NOLINT
        stream << ('0');
    }
...
boost::posix_time::time_input_facet* Utc::CreateDateFormatIn(std::string_view pattern) {
    return new boost::posix_time::time_input_facet(std::string(pattern));
}
...
std::shared_ptr<Utc> Utc::Parse(const std::string_view text, std::string_view pattern) {
    ...
    if (dot_pos != std::string::npos) {
        ...
        if (fraction_string.length() > 6) {  // max. 6 digits!  // NOLINT
        ...
        for (auto i = fraction_string.length(); i < 6; i++) {  // NOLINT
            micros *= 10;                                      // NOLINT
        }
    }
