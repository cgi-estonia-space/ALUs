...
double Ellipsoid::ComputeEllipsoidNormal(const double phi) { return A / sqrt(1.0 - E2 * pow(sin(phi), 2)); }
// double Ellipsoid::ComputeCurvatureRadiusInMeridianPlane(const double phi) {
//     const double power_to_square_root_of_cube{1.5};
//     return a_ * (1 - E2) / pow((1 - E2 * pow(sin(phi), 2)), power_to_square_root_of_cube);
// }
...
//     Ellipsoid::e2_ = DEFAULT_FIRST_ECCENTRICITY;
//     Ellipsoid::e2b_ = DEFAULT_SECOND_ECCENTRICITY;
...
s1tbx::Point Ellipsoid::Ell2Xyz(const double phi, const double lambda, const double height) {
    ...
    return s1tbx::Point(a * cos(lambda), a * sin(lambda), (nph - E2 * n) * sin(phi));
}
...
//     return Ell2Xyz(geo_point.lat_ * snapengine::eo::constants::DTOR, geo_point.lon_ *
//     snapengine::eo::constants::DTOR,
//                    height);
// }
