...
using alus::snapengine::geoutils::Geo2xyzWgs84;
...
TEST_F(GeoUtilsTest, geo2xyzWGS84ComputesCorrectly) {
    ...
    std::vector<PosVector> const expected_results{{3089569.9005578943, 1261476.4569731771, 5417255.575817931},
    ...
    ASSERT_EQ(expected_results.size(), lats.size());

    for (size_t i = 0; i < lats.size(); i++) {
        PosVector pos{};
        Geo2xyzWgs84(lats.at(i), lons.at(i), alts.at(i), pos);
        auto const expected = expected_results.at(i);
